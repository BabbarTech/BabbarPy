...
def host_fetches(host, lang, API_KEY, data=[""]):
    a = 0
    data_list = []
        ...
        if resp.status_code != 200:
            ...
            try:
                ...
                if isinstance(aList, list):
                    # Ajouter une condition de sortie lorsque la liste est vide
                    if not aList:  
                        print("No more data.")
                        break
...
def host_fetches_to_csv(hosts_file, lang, API):
    data_list = []
    with open(hosts_file, 'r') as f:
        hosts = [line.strip() for line in f]
        for host in hosts:
            data_list.extend(host_fetches(host, lang, API))
...
def main():
    ...
    host_fetches_to_csv(hosts_file, 'all', api_key)
